...
Our products site, with its php based frontend and the API backend, can be deployed using two type of Pods, each Pod running a single container, each container running a single process.
...
[TIP]
.Too Lazy to Build?
Images can be pulled directly at: +
...
First, create `backend-deployment.yaml` and `backend-service.yaml`:

[source,yaml]
----
include::backend-deployment.yaml[backend-deployment.yaml]
----

[source,yaml]
----
include::backend-service.yaml[backend-service.yaml]
----
...
=== 3. Packaging and Upgrading your K8s packages using Helm Chart
...
Helm is the best fit out for this in the context of k8s.
...
For testing the app we should first forward ports of either __frontend__ or __backend__ Pod:
